...
namespace base {
        ...
        LibLogFunctions *GetLibLogFunctionsInternal() {
            void *liblog_handle = dlopen("liblog.so", RTLD_NOW);
            if (liblog_handle == nullptr) {
                return nullptr;
            }

            auto* real_liblog_functions = new LibLogFunctions() ;
...
const LibLogFunctions* GetLibLogFunctions() {
  static LibLogFunctions*  liblog_functions = GetLibLogFunctionsInternal();
...
const LibLogFunctions& GetLibLogFunctions() {
  static LibLogFunctions liblog_functions = []() -> LibLogFunctions {
